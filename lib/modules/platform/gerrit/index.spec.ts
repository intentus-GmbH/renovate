...
import { GerritChange, TAG_PULL_REQUEST_BODY } from './types';
...
describe('modules/platform/gerrit/index', () => {
  ...
  describe('isBranchModified()', () => {
    it('no open change for with branchname found -> not modified', () => {
      httpMock
        .scope(gerritEndpointUrl)
        .get('/a/changes/')
        .query(
          (query) =>
            (query?.q?.includes('status:open') &&
              query?.q?.includes('hashtag:sourceBranch-myBranchName')) ??
            false
        )
        .reply(200, gerritRestResponse([]), jsonResultHeader);
      return expect(
  ...
  describe('isBranchBehindBase()', () => {
    it('no open change for with branchname found -> isBehind == true', () => {
      httpMock
        .scope(gerritEndpointUrl)
        .get('/a/changes/')
        .query(
  ...
  describe('isBranchConflicted()', () => {
    it('no open change for with branch name found -> throws an error', () => {
      httpMock
        .scope(gerritEndpointUrl)
        .get('/a/changes/')
        .query(
          (query) =>
            (query?.q?.includes('hashtag:sourceBranch-myBranchName') &&
              query?.q?.includes('branch:target')) ??
            false
        )
        .reply(200, gerritRestResponse([]), jsonResultHeader);
      return expect(
    ...
    it('open change found for branch name/baseBranch and its mergeable', () => {
      ...
      httpMock
        .scope(gerritEndpointUrl)
        .get(`/a/changes/${change._number}/revisions/current/mergeable`)
        .reply(
          200,
          gerritRestResponse({
    ...
    it('open change found for branch name/baseBranch and its NOT mergeable', () => {
      const change: GerritChange = Fixtures.getJson('change-data.json');
      httpMock
        .scope(gerritEndpointUrl)
        .get('/a/changes/')
  ...
  describe('branchExists()', () => {
    it('no change found for branch name -> return result from git.branchExists', () => {
      ...
      httpMock
        ...
        .reply(200, gerritRestResponse([]), jsonResultHeader);
      return expect(gerrit.branchExists('myBranchName')).resolves.toBeTrue();
    });

    it('open change found for branch name -> return true', () => {
      const change: GerritChange = Fixtures.getJson('change-data.json');
      httpMock
        .scope(gerritEndpointUrl)
        .get('/a/changes/')
  ...
  describe('getBranchCommit()', () => {
    it('no change found for branch name -> return result from git.getBranchCommit', () => {
  ...
  describe('commitFiles()', () => {
    ...
    it('commitFiles() - create first Patch', async () => {
      ...
      expect(git.prepareCommit.mock.calls[0][0].message[0]).toBe('commit msg');
      expect(git.prepareCommit.mock.calls[0][0].message[1]).toMatch(
        /Change-Id: I.{32}/
      );
    ...
    it('commitFiles() - existing change-set without new changes', async () => {
      ...
      expect(res).toBeNull();
      expect(git.prepareCommit.mock.calls[0][0].message).toEqual([
        'commit msg',
        'Change-Id: ...',
      ]);
